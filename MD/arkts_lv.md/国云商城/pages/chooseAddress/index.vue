...
<script>
	...
	const qqmapsdk = new QQMapWX({
		key: 'CWVBZ-IPLHI-JPFGP-53TJM-566NH-M2BNK'
	})
	let _mapCtx = null;
	export default {
		data() {
		...
		methods: {
			...
			submit() {
				...
				uni.setStorageSync("address",this.currentAddress.address+" "+this.room)
				uni.navigateBack();
			},
			//获取poi列表  
			getAddressList(s = 0) {
			...
			getLocation (){
				return new Promise(resolve=> {
					...
					uni.request({
						method:"GET",
						url:"http://121.5.53.212:3000/location",
						data:{
							key:"QTQBZ-ZDXWF-YMCJM-NTTLD-S2QSV-HJF32"
						},
						success: (res) => {
