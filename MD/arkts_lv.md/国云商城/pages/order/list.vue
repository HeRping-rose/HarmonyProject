<template>
	<view class="app">
		...
		z-index: 10000000; background: #fff;">
			<my-header></my-header>
		</view>
		<view class="nav-wrap">
			...
			<view class="item column" v-for="(item, index) in list" :key="index" @click="navTo('detail?id='+item._id)">
				<view class="i-header row b-b">
					<text class="time fill">{{ item.createTime | date }}</text>
					<text class="state" :class="{gray:item.status===10}">{{ item.state | state }}</text>
				</view>
				<!-- 单商品 -->
				<view v-if="item.order.length === 1" class="goods-wrap row">
					<view class="image-wrapper lazyload lazypic" :class="{loaded: item.loaded}">
						<image :src="URL+item.order[0].pic" mode="aspectFill" lazy-load="true"  ></image>
					</view>
					<view class="right column">
						<text class="title clamp">{{ item.order[0].name }}</text>
						<text class="attr">{{ item.order[0].size }}</text>
						<view class="r-b">
							<text class="price">￥{{ item.order[0].price }}</text>
							<text class="number">x{{ item.order[0].count }}</text>
						</view>
					</view>
				</view>
				<!-- 多商品 -->
				<scroll-view v-else class="scroll-view" scroll-x>
					<view class="goods-list">
						<view v-for="(gItem, gIndex) in item.order" :key="gIndex" class="image-wrapper lazyload lazypic" :class="{loaded: item.loaded}">
							<image :src="URL+gItem.pic" mode="aspectFill" lazy-load="true"  ></image>
						</view>
					</view>
				</scroll-view>
				<view class="price-wrap">
					<text>共{{ item.order.reduce((t,i)=>t+=i.count,0) }}件 实付:</text>
