...
class Manage:
    ...
    def showStudent(self):
        print("姓名\t性别\t电话")
        for student in self.studentList:
            print(f"{student.name}\t{student.gender}\t{student.phone}")

    def saveStudent(self):
        f = open("info.data","w")
        # info = []
        # for student in self.studentList:
        #     info.append(student.__dict__)
        info = [student.__dict__ for student in self.studentList]
        f.write(str(info))
        f.close()
    ...
    def loadStudent(self):
        try:
            ...
            #eval()用于把字符串，转成正常表达式
            info = eval(f.read())
            self.studentList = [Student(i['name'],i['gender'],i['phone']) for i in info]
        finally:
            f.close()
